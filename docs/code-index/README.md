...
|---|---|---|---|
| `index.html` | ≈1.35 тыс. | Вся разметка: loader, 5 экранов, dock, модалки | `index.html.md` |
| `tokens.css` | ≈70 | Палитра, шрифты, радиусы — единственный источник цветов | `css.md` |
| `main.css` | ≈9 тыс. | Основные стили всех экранов, слои «final priority» | `css.md` |
| `reactivation-v30.css` | ≈5 тыс. | Слой хотфиксов поверх main.css (не только Reactivation), почти всё под `[data-theme="dark"]` | `css.md` |
| `main.js` | ≈7 тыс. | Всё поведение: данные, рендер, навигация, окно, API | `main.js.md` |
| `Code.gs` | ≈4 тыс. | Apps Script API над Google Sheet | `Code.gs.md` |
...
| `tools/visual-check/` | — | Стенд скриншот-диффа (мок API + Playwright) | `tools/visual-check/README.md` |
...
| Yesterday/Dashboard: считаются на клиенте из 365-профилей клиентов | `buildAnalyticsFrom`, `refreshAnalyticsFrom` | `build365StatisticsProfiles_` |
